...
async function actualizarEstadoTarea(taskId, nuevoEstado, selectElement, tdElement) {
  ...
  try {
    ...
    if (tdElement) {
      ...
      else if (nuevoEstado === 'Completado') tdElement.classList.add('status-completed');
    ...
    datos.forEach(filaData => {
      ...
      columnas.forEach(columna => {
        ...
        if ((columnaStr === 'estado' || columnaStr === 'status') && filaData.id) {
          ...
          else if (valor === 'Completado') celda.classList.add('status-completed');
      ...
      const tituloEliminar = document.createElement('th');
      ...
      encabezadoFila.appendChild(tituloEliminar);
